...
async function getProductsSafe() {
  try {
    ...
    const data = await response.json();
    if (response.ok) return { success: true, data: data.products };
    else return { success: false, error: "取得產品資料失敗" };
  } catch (error) {
...
1. HTTP 狀態碼的分類（1xx, 2xx, 3xx, 4xx, 5xx 各代表什麼）
   答：1xx: 資訊性質的狀態碼，表示伺服器收到請求，正在處理中。
       2xx: 成功
       3xx: 重定向狀態碼
       4xx: 客戶端錯誤
       5xx: 後端錯誤
